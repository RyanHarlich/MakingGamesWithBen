...
// When making a new version, add it here
const unsigned int TEXT_VERSION_0 = 100;
// Example if there was another version
//const unsigned int TEXT_VERSION_1 = 101;
...
bool LevelReaderWriter::saveAsTextV0(const std::string & filePath, const Player & player, const std::vector<Box>& boxes, const std::vector<Light>& lights) {
	...
	// Write version, this has to be TEXT_VERSION_0 because of the function saveAsTextV0 not TEXT_VERSION which can change to another version
	file << TEXT_VERSION_0 << '\n';
	...
	for (auto& b : boxes) {
		file << b.getPosition().x << ' ' << b.getPosition().y << ' '
			...
			<< b.getIsDynamic() << ' ' << b.getFixesRotation() << '\n';
	}
...
bool LevelReaderWriter::loadAsTextV0(std::ifstream & file, b2World * world, Player & player,  std::vector<Box>& boxes, std::vector<Light>& lights) {
	...
	{ // Read player
		glm::vec2 pos;
		glm::vec2 ddims; // draw
		glm::vec2 cdims; // collision
		Bengine::ColorRGBA8 color;
		bool fixedRoation = true;
		file >> pos.x >> pos.y >> ddims.x >> ddims.y >> cdims.x >> cdims.y >> color.r >> color.g >> color.b >> color.a;
		player.init(world, pos, ddims, cdims, color, fixedRoation);
	}
